...
import { Home, PlusCircle, Heart, User, Sparkles } from 'lucide-react';
...
export const BottomNav: React.FC = () => {
  ...
  // Simplified to 4 essential items - all other actions in hamburger menu
  const navItems = [
    {
      id: 'home',
      icon: Home,
      ...
      <div className="flex items-center justify-around px-2 py-2">
        ...
        {navItems.filter(item => item.id === 'home').map((item) => {
          ...
          return (
            <Link
              ...
              to={item.href}
              className={cn(
                "relative flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-2xl transition-all duration-300 min-w-[56px]",
                active && "bg-gradient-to-br from-gray-100 to-gray-50"
              )}
            >
              <div className={cn(
        ...
        {navItems.filter(item => item.isMain).map((item) => {
          ...
          return (
            ...
            >
              <div className={cn(
                "w-14 h-14 -mt-6 rounded-2xl flex items-center justify-center shadow-xl border-4 border-white transition-all duration-300 relative",
        ...
        {/* Last two items (Favorites & Profile) */}
        {navItems.filter(item => item.id === 'favorites' || item.id === 'profile').map((item) => {
          const Icon = item.icon;
          const active = isActive(item.href);
