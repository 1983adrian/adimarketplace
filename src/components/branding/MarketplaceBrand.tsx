...
export const MarketplaceBrand: React.FC<MarketplaceBrandProps> = ({
  ...
  const BrandContent = () => (
    <div className={cn('flex flex-col items-center', className)} style={{ gap: 0 }}>
      {/* Hero Logo - seamless, no borders, no margins */}
      <div className="w-full flex items-center justify-center">
        <img 
          src={heroLogo} 
          alt="MarketPlace România" 
          className={cn(
            'object-contain m-0 p-0 block',
            size === 'sm' && 'h-36 sm:h-40',
            size === 'md' && 'h-48 sm:h-56',
            size === 'lg' && 'h-60 sm:h-72 md:h-80',
            size === 'xl' && 'h-52 sm:h-64 md:h-[22rem] lg:h-[26rem]',
          )}
          style={{ 
      ...
      {showTagline && (
        <p className={cn(
          'font-semibold tracking-wide text-center -mt-8 sm:-mt-10 relative z-10',
          taglineSizes[size]
        )}>
          <span className="text-[#4285F4]">{t('brand.sell')}</span>
          <span className="mx-1.5 text-muted-foreground/50">•</span>
          <span className="text-[#FCD116]">{t('brand.buy')}</span>
          <span className="mx-1.5 text-muted-foreground/50">•</span>
